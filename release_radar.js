...
// @downloadURL https://update.greasyfork.org/scripts/510955/Deezer%20Release%20Radar.user.js
// @updateURL https://update.greasyfork.org/scripts/510955/Deezer%20Release%20Radar.meta.js
// ==/UserScript==
...
function time_ago(unix_timestamp) {
    ...
    const difference = Date.now() - Math.abs(unix_timestamp);
    const diff_test = Math.abs(difference);
    ...
    if( time_ago_formatter !== undefined ) {
      return time_ago_formatter.format(-time_ago, unit);
    } else {
      returned = `${time_ago} ${pluralize(unit, time_ago)}`;
    }

    if( Date.now() - unix_timestamp < 0) {
       returned = `In ${returned}`
    } else {
       returned = `${returned} ago`
    }
    return returned;
}
...
.release_radar_main_scroll {
    max-height: 450px;
    overflow-y: auto;
}

.release_radar_upcoming_div_wrapper {
    padding: 5px 0px;
    width: 100%;
}
.release_radar_upcoming_div_wrapper summary {
    padding: 0 16px;
}
    ...
    function create_release_li(release) {
        ...
        if (!cache.has_seen[release.id]) {
            ...
            release_li.onmouseover = () => {
                ...
                if (amount_new_songs <= 0 && amount_songs_span) {
...
function create_main_div() {
    ...
    const main_div = document.createElement("ul");
    main_div.className = "release_radar_main_div"

    const upcoming_div = document.createElement("details");
    upcoming_div.className = "release_radar_upcoming_div_wrapper hide"
    const upcoming_summary = document.createElement("summary");
    upcoming_summary.textContent = "Upcoming releases";
    const upcoming_list = document.createElement("ul");
    upcoming_list.className = "release_radar_upcoming_div"
    upcoming_div.append(upcoming_summary, upcoming_list);
    ...
    const main_scroll = document.createElement("div");
    main_scroll.className = "release_radar_main_scroll";
    main_scroll.append(upcoming_div,main_div);

    popper_div.append(header_wrapper_div, main_scroll, last_checked_span);
    wrapper_div.append(popper_div, arrow_div);
    return [wrapper_div, main_div, upcoming_list, upcoming_div, upcoming_summary];
}
...
async function main() {
    ...
    if( Intl !== undefined && Intl.RelativeTimeFormat !== undefined ) {
      time_ago_formatter = new Intl.RelativeTimeFormat(user_data.results.SETTING_LANG, {
    ...
    function create_ui(parent) {
        ...
        const [wrapper_div, main_div, upcoming_list, upcoming_div, upcoming_summary] = create_main_div();
        ...
        const wait_for_releases_data = setInterval(async () => {
            ...
            if (new_releases) {
                ...
                log("Got data");
                const current_time = Date.now();
                const upcoming = [...new_releases].filter(function(release) {
                  return release.release_date > current_time === true;
                });
                const released = [...new_releases].filter(function(release) {
                  return release.release_date > current_time === false;
                });
                log(`${upcoming.length} upcoming, ${released.length} already released`);

                if( upcoming.length ) {
                  const new_releases_divs = create_new_releases_lis(upcoming, main_btn, upcoming_list, user_data.results.SETTING_LANG);
                  upcoming_list.append(...new_releases_divs);
                  upcoming_summary.textContent += ` (${upcoming.length})`;
                  upcoming_div.classList.remove("hide");
                }
                if( released.length ) {
                  const new_releases_divs = create_new_releases_lis(released, main_btn, wrapper_div, user_data.results.SETTING_LANG);
